...
#include "audio_processor_postcondition.h"
...
void AudioService::Stop() {
    ...
    bool notify_drained = false;
    {
        ...
        stream_generation_.BumpPlayback();
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        if (!audio_decode_queue_.empty() &&
            ...
            lock.lock();
            if (decoded && stream_generation_.AcceptPlayback(generation) &&
...
bool AudioService::IsAudioProcessorRunning() const {
    AudioEngine* engine = audio_engine_.get();
    const bool initialized = audio_engine_initialized_;
    const bool present = engine != nullptr;
    const bool engine_enabled = present && engine->IsVoiceProcessingEnabled();
    const bool event_running = event_group_ != nullptr && (xEventGroupGetBits(event_group_) &
                                                           AS_EVENT_AUDIO_PROCESSOR_RUNNING) != 0;
    // Load Stop last so a concurrent Stop() cannot be observed as running via
...
bool AudioService::EnableVoiceProcessing(bool enable) {
    ...
    if (!enable) {
        ...
        if (audio_engine_initialized_ && audio_engine_ != nullptr) {
            audio_engine_->EnableVoiceProcessing(false);
            engine_enabled = audio_engine_->IsVoiceProcessingEnabled();
        }
        if (VoiceProcessingDisableKeepServiceEvent(engine_enabled)) {
            ESP_LOGW(TAG, "Voice processing disable left the engine enabled");
            if (!service_stopped_.load()) {
        ...
        const bool event_running =
            event_group_ != nullptr &&
            (xEventGroupGetBits(event_group_) & AS_EVENT_AUDIO_PROCESSOR_RUNNING) != 0;
        return VoiceProcessingDisableSucceeded(engine_enabled, event_running);
    }

    if (service_stopped_.load() || !InitializeAudioEngine() || !audio_engine_initialized_ ||
    ...
    if (!VoiceProcessingEnableMayPublishEvent(service_stopped_.load(),
                                              audio_engine_->IsVoiceProcessingEnabled())) {
        ESP_LOGW(TAG, "Voice processing enable was not accepted by the audio engine");
        audio_engine_->EnableVoiceProcessing(false);
        xEventGroupClearBits(event_group_, AS_EVENT_AUDIO_PROCESSOR_RUNNING);
        return false;
    }
...
void AudioService::ResetStreamingState() {
    EnableVoiceProcessing(false);
    bool notify_drained = false;
    {
        std::lock_guard<std::mutex> lock(audio_queue_mutex_);
        stream_generation_.BumpStreaming();
        audio_encode_queue_.clear();
