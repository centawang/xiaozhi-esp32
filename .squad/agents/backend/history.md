...
## 2026-09-13T01:46:42.395Z — stroke-interaction-improvements-audio-revision
...
- TryPlayUiFeedback must not pass mutex-protected non-atomic fields as call arguments after try_to_lock; C++ evaluates arguments before the callee can reject a failed lock. Return on !owns_lock() or invoke protected reads only inside a lambda gated by lock_acquired.
- A 2-slot playback ring shared with UI feedback needs an explicit ordinary reservation while decode_in_flight_ is true. UI may use only unreserved capacity; TryPushOrdinary must take unique_ptr&& and leave the caller owning the task on capacity failure.
- Feature-on UI playback references service-owned PCM, so AudioService destruction must Stop() then join workers (null handles = never-started no-op) before destroying ui_feedback_pcm_, mutex/CV, timer, and the event group. Stop() itself must not join on the audio hot path.
