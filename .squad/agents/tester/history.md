...
<!-- pi-squad:990f92fc3dd23e265859ce914f6628e94ab5b6c661fa380b9c1286e02be51af3 -->
## 2026-09-12T17:34:39.870Z — stroke-2000-sharded-library-review

# stroke-2000-sharded-library 独立审查
...
- 真实 fixture 为一级字表连续 rank 1–2000，8 个 SOB1 shard 各 250 字，总计 5,800,492 B；SCB1 为 24,352 B，SHA-256 为 91779189b45526ab47cff2927b8da35e345e967a84e72096221e658e2cc919e4。
- SPY1 实际为 63,618 B、2000 characters、1047 groups、最大 group 29；Python/C++ 上限同步为 2048 characters/groups，文件上限仍为 65,536 B。
- 生产 Controller 不复制完整 5.8 MiB corpus；它拥有 SCB1/SPY1 metadata 和已解码 glyph，shard 仅在 RAII Acquire/Release 作用域内映射。
- RebindStrokeOrderAssets 会在全量验证 8 shards 并核对 SCB1/SPY1 的全部 codepoint/rank 后才 ready；Suspend 在 assets munmap 前同步清除 View/Controller/pinyin 状态。
- runtime stroke pack 固定为 15 个文件，SHA256SUMS 对其余 14 项闭合覆盖；selection、charset、coverage、source/per-shard manifests 不进入固件 assets。
- 容量门禁使用真实 assets partition size，要求 image 严格小于分区且余量至少 262,144 B；最终 clean build 的余量为 820,401 B。
- 最终留存 host 输出为 127 tests、OK、无 skip；专项 sharded C++ harness 输出 max_active=1、max_bytes=911688。
- ESP-IDF v6.0.2 CoreS3 clean build 成功；xiaozhi.bin 为 2,914,800 B，generated_assets.bin 为 7,568,207 B，对应 SHA-256 分别为 48fbf45cddc2a87ecf7e38cb10b21cd1326b88cdeca37a06371c79461c75b7d0 与 54453b5232408dfb8cf8dadbde9dd5d93a1bdb90f213df69d7ff075210783dd0。
- WebSocket 仍是 local-stroke 构建的语音路径；MQTT 明确不提供 correlated stroke voice，只做不打开音频通道的本地候选降级，session/generation fail-closed 逻辑仍由回归 harness 覆盖。
- 该 2000 字数据仍是技术原型：transcription aid 无明确仓内许可证，尚无双人官方 PDF 核对、逐字笔顺审核或商业发布法务结论。

<!-- pi-squad:df90a450f9494264fe12c6216e6eb88d385f54edb73d4d12e8049b6ecbc35ca4 -->
## 2026-09-12T18:01:50.328Z — stroke-final-core-s3-2000-retry
...
- 现有 2000 字包目录精确含 56 个文件；顶层 SHA256SUMS 闭合覆盖其余 55 个文件，runtime/audit 内层校验分别覆盖 14/28 个 payload。
- ZIP 大小为 38,449,270 bytes，SHA-256 为 `7eb600eba44509ccb3c6666323d6cd884172e5447482c74121a4a2c74aec7773`；ZIP CRC、解压 checksum 和原目录逐字节比较均通过。
- CoreS3 镜像的 app 为 2,914,784 bytes，generated assets 为 7,568,207 bytes，merged 为 15,956,815 bytes；assets/flash 末端余量均为 820,401 bytes。
