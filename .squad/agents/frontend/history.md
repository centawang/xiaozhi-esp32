...
## 2026-09-13T01:18:00.491Z — stroke-interaction-improvements-revision-retry
...
- StrokeOrderController::Tick(dt_ms) must consume remaining time across 1000 ms reveal and 160 ms gap phases in a loop bounded by kMaxStrokesPerCharacter*2+2; leftover after the last stroke completes without starting a gap, and paused/completed Tick is a no-op.
- CONFIG_STROKE_ORDER_LOCAL UI feedback admission must enqueue a generation-bearing token into a fixed 2-slot ring shared with ordinary decode PCM, playing an immutable service-owned 40 ms tone by reference; TryPlayUiFeedback uses try_to_lock and fail-silent rejection with no heap/PCM copy and no ResetDecoder/generation bump.
- A valid feature-disabled CoreS3 compile requires an outside-repo SDKCONFIG; the in-repo build/CMakeCache can still point at a previous feature-on /tmp sdkconfig even when the tracked sdkconfig has CONFIG_STROKE_ORDER_LOCAL unset.
- Feature-enabled ESP-IDF 6.0.2 CoreS3 firmware from /tmp/xiaozhi-so-on-build: xiaozhi.bin SHA-256 51c7de0862293ef3bbf6bf16afc23618e93fb4fd6b2e35de43553c801381d1bf, merged-binary.bin SHA-256 a2f518f3dd38261d1f5cdbc64cbc9fac97d2233e23f25d17f05d50c7c955c4bd, generated_assets.bin SHA-256 54453b5232408dfb8cf8dadbde9dd5d93a1bdb90f213df69d7ff075210783dd0.
